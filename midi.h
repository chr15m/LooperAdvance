...
inline void midiInit()
{
	...
	REG_TM0CNT_H = TIMER_START | TIMER_IRQ;   // start + irq + f (16MHz)
}
...
inline void midiInterrupt(void) {
	...
	if (midi_send_struct.byte >= midi_send_struct.data_size)
		...
		bit = 1 & (midi_send_struct.data[midi_send_struct.byte] >> midi_send_struct.next_bit);
		midi_send_struct.next_bit++;
	}
	
	// perform the cable send
	REG_RCNT = (bit) ? REG_R_MIDI | GPIO_SC : REG_R_MIDI;
...
u8 midi_message_note[] = {0x90, 0x00, 0x00};
...
u8 midi_message_spp[] = {0xF2, 0x00, 0x00};
u32 lastbeat = 0;
